...
public class DialogPrompt extends DialogFragment {
    ...
    String confirmationCode, surname, firstname, phoneNo, enterPassword, email, userId;
    ArrayList<String> finalContactsList, foundVendors;
    Bundle bundle;
    String rawPassword;
    SQLiteDatabase mydatabase;

    @SuppressLint("ValidFragment")
    public DialogPrompt(String extra, String surname, String firstname, String phoneNo, String password, String email, String userId){
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        finalContactsList = bundle.getStringArrayList("val2");
        foundVendors = bundle.getStringArrayList("val3");
        //Toast.makeText(getActivity().getApplicationContext(),"Received "+finalContactsList.size()+" contacts",Toast.LENGTH_SHORT).show();
    ...
    private class permUserSignUp extends AsyncTask<String,Void,String> {
        Context context;

        public permUserSignUp(Context c){
            context = c;
        }

        @Override
        protected String doInBackground(final String... params) {
            String dataUrl = "http://workchopapp.com/mobile_app/perm_user_signup.php";
            String dataUrl2 = "http://workchopapp.com/mobile_app/clear_temp_user_table.php";
                ...
                h.post(new Runnable() {
                    public void run() {
                        if(sb.toString().split("--")[0].equals("done")) {
                            ...
                            intent.putStringArrayListExtra("finalContactsList", finalContactsList);
                            intent.putStringArrayListExtra("foundVendors", foundVendors);
                            for(int i=0; i< foundVendors.size(); i++){
                                //Log.v("A FOUND VENDOR",foundVendors.get(i));
                            }
    ...
    private class getRegistrationCode extends AsyncTask<String,Void,String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try{
                ...
                String dataUrl3 = "http://workchopapp.com/mobile_app/get_temp_user_details.php";
                ...
                firstname = sb3.toString().split("--")[2];
                phoneNo = sb3.toString().split("--")[3];
                email = sb3.toString().split("--")[4];
                userId = sb3.toString().split("--")[5];
                ...
                h.post(new Runnable() {
                    public void run() {
                        ...
                        if(confirmationCodeText.getText().toString().equals(confirmationCode)) {
                            ...
                            new permUserSignUp(getActivity().getApplicationContext()).execute(surname,firstname,email,phoneNo,userId,
                                    enterPassword);
                        }
                        else{
                            Toast.makeText(getActivity().getApplicationContext(), "Wrong Confirmation Code", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
