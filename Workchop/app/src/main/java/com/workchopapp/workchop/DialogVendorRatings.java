...
public class DialogVendorRatings extends DialogFragment {
    AlertDialog dialog;
    Bundle bundle;
    String vendorId;
    String pointsCount="";
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        final View view = getActivity().getLayoutInflater().inflate(R.layout.dialog_venodor_ratings, null);
        builder.setView(view);
        bundle = getArguments();
        vendorId = bundle.getString("vendorId");
        pointsText = (TextView)view.findViewById(R.id.ratingsText);
        Typeface type = Typeface.createFromAsset(getActivity().getAssets(),"fonts/GOTHIC.TTF");
        pointsText.setTypeface(type);
        new getPoints(getActivity().getApplicationContext()).execute(vendorId);
    ...
    private class getPoints extends AsyncTask<String,Void,String> {
        ...
        protected String doInBackground(String... params) {
            String dataUrl = "http://workchopapp.com/mobile_app/get_vendor_user_ratings.php";
                ...
                h.post(new Runnable() {
                    public void run() {
                        if(sb.toString().contains("------")){
                            pointsText.setText("Your average rating is "+sb.toString().split("------")[1]+" out of 5.0 by " +
                                    sb.toString().split("------")[0] +" users");
                        }
                        else{
                            pointsText.setText("Error loading Ratings.");
                        }
